...
const CartItem = ({ onContinueShopping }) => {
  ...
  const handleContinueShopping = (e) => {
    e.preventDefault();
    onContinueShopping(); // Call the continue shopping function passed as a prop
  };

  const handleIncrement = (item) => {
    dispatch(updateQuantity({ id: item.id, quantity: item.quantity + 1 })); // Increment item quantity
  };

  const handleDecrement = (item) => {
    if (item.quantity > 1) {
      dispatch(updateQuantity({ id: item.id, quantity: item.quantity - 1 })); // Decrement item quantity
    } else {
  ...
  const handleRemove = (item) => {
    dispatch(removeItem(item.id)); // Remove item from cart
  };
